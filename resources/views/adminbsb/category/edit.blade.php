...
@section('action')
    <div class="row">
        <div class="col-sm-12">
            <form id="frmCategory" action="{{ url('/'.BULK_SYSTEM.'/'.$controller.'/'.$category->category_id) }}" method="POST"  class="form-horizontal" role="form" novalidate="novalidate">
                {{ csrf_field() }}
                {{ method_field('PATCH') }}
                <div class="row">
                    <div class="col-sm-12">
                        ...
                        <div class="col-sm-12">
                            <div class="form-group">
                                <div class="form-line">
                                    <input type="text" name="category_name" class="form-control limit limit255" value="{{ $category->name }}" />
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="row">
                    <div class="col-sm-4">
                        ...
                        @include('adminbsb.partials.select_picture', ['picture' => $category->picture])
                    </div>
                    <div class="col-sm-1"></div>
                    <div class="col-sm-3">
                        <label class="form-label">{{ trans('adminbsb.category_status') }}</label>
                        @include('adminbsb.partials.switch_status', ['module' => 'category', 'switchStatus' => $category->status])
                    </div>
                    <div class="col-sm-1"></div>
                    <div class="col-sm-3">
                        <label class="form-label">{{ trans('adminbsb.category_order') }}</label>
                        @include('adminbsb.partials.order', ['orderValue' => $category->order])
                    </div>
                </div>
                <div class="row">
                    <div class="col-sm-12">
                        ...
                        <div class="col-sm-12">
                            <div class="form-group">
                                <div class="form-line">
                                    <textarea name="category_desc" class="form-control limit limit1024 tinyEditor">{{ $category->description }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
